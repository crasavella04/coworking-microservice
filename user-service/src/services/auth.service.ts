...
import { AuthImplements } from 'src/interfaces/implements/auth.implements';
...
export class AuthService implements AuthImplements {
  ...
  async registration(dto: CreateUserDto) {
    try {
      ...
      if (isValidPassword !== true)
        throw new HttpException(isValidPassword, HttpStatus.NOT_ACCEPTABLE);
  ...
  async refresh(token: string) {
    const { id, email } = this.tokenService.decodeAccessToken(
      token.split(' ')[1],
    );
